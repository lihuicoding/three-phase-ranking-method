...
close all
clear all
X=xlsread('C:\Users\Administrator\Desktop\工作 安大写文章\工作第2篇- 准则偏好下的三支多准则决策方法\程序\本身\实验分析\实验—有序分类—萨尔维莫幸福\Somerville','B2:G144'); 
J = [1 1 1 1 1 1];
[m,n] = size(X);
A=zeros(m,n);
for j = 1:n
...
s=xlswrite('paixu_Somerville.xls', result);  % 将result写入到 文件中


RO=relative_volatility(data');
...
title3 = {'本文方法','WA','TOPSIS', 'Zhang202138413856_jue','Zhang202138413856_xiang','Jia1', 'Jia2','Jia3'};  % 添加变量名称
result3 = [title3; data_cell3];   % 将变量名称和数值组集到result
s3=xlswrite('volatility_Somerville.xls', result3);    % 将result写入到文件中
...
title2 = {'本文方法','WA','TOPSIS', 'Zhang202138413856_jue','Zhang202138413856_xiang','Jia1', 'Jia2','Jia3'};  % 添加变量名称
result2 = [title2; data_cell2];   % 将变量名称和数值组集到result
s2=xlswrite('Spearman_Somerville.xls', result2);    % 将result写入到paixu.xls文件中