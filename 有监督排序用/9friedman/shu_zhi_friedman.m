...
close all
clear all
X=xlsread('C:\Users\Administrator\Desktop\数据增加实验\实验—回归—friedman\friedman','A11:E1210');
J = [1 1 1 1 1];
[m,n] = size(X);
A=zeros(m,n);
for j = 1:n
...
W=ones(1,n).*(1/n);
%W=shang_quan(RTR);
X1=xlsread('C:\Users\Administrator\Desktop\数据增加实验\实验—回归—friedman\friedman','F11:F1210');
...
s=xlswrite('paixu_friedman.xls', result);  % 将result写入到paixu.xls文件中
...
s3=xlswrite('volatility_friedman.xls', result3);    % 将result写入到文件中
...
s1=xlswrite('weizhi_friedman.xls', result1);    % 将result写入到paixu.xls文件中
...
s2=xlswrite('Spearman_friedman.xls', result2);    % 将result写入到paixu.xls文件中
