...
close all
clear all
X=xlsread('C:\Users\Administrator\Desktop\数据增加实验\实验—回归—wizmir\wizmir','A1:I1461');
J = [1 1 1 2 2 1 1 1 2];
[m,n] = size(X);
A=zeros(m,n);
for j = 1:n
...
W=ones(1,n).*(1/n);
%W=shang_quan(RTR);
X1=xlsread('C:\Users\Administrator\Desktop\数据增加实验\实验—回归—wizmir\wizmir','J1:J1461');
...
s3=xlswrite('volatility_wizmir.xls', result3);    % 将result写入到文件中
...
s2=xlswrite('Spearman_wizmir.xls', result2);    % 将result写入到paixu.xls文件中
